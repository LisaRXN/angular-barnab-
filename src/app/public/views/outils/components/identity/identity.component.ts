...
export class IdentityComponent {
  ...
  confirmationStep() {
    if (!this.checkCGU) {
      this.errorMessage =
        "Veuillez acceptez la politique d'utilisation des données personnelles.";
      return;
    }
    ...
    if (
      ...
      this.tools === 'redaction'
        ? this.router.navigate([`/redaction-annonce-immobiliere/confirmation`])
        : this.router.navigate([
            `/estimation-immobiliere-gratuite/confirmation`,
          ]);
    }
  }
